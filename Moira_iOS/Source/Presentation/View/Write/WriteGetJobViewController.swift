//
//  WriteGetJobViewController.swift
//  Moira_iOS
//
...
class WriteGetJobViewController: UIViewController,StoryboardView {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        reactor = DependencyProvider().container.resolve(WriteGetJobViewReactor.self)!
    }
    
    func bind(reactor: WriteGetJobViewReactor) {
        applyButton.rx.tap
            .map { [unowned self] _ in
                let request = PostJobRequest(title: titleLabel.text!,
                                             field: field.text!,
